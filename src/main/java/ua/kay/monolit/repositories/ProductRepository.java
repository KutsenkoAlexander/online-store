...
public interface ProductRepository extends PagingAndSortingRepository<Product, Long> {
    ...
    @Async
    @Query("select p from ImgProduct p where p.productByProductId.categoryId = ?1")
    Page<Product> findByCategoryId(Integer categoryId, Pageable pageable);

    @Async
    @Query("select p from ImgProduct p where p.productId = ?1")
    ImgProduct findByIdProduct(Long productId);
    ...
    @Async
    @Query("select p from ImgProduct p where (p.productByProductId.categoryId = ?1 or ?1 is null) " +
